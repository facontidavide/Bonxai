  ...
  if (create_if_missing) {
    if (!inner_ptr->mask().setOn(inner_index)) {
      if constexpr (std::is_trivial_v<DataT> && !std::is_same_v<DataT, EmptyVoxel>) {
        ...
        inner_data = std::make_shared<LeafGrid>(mutable_grid_.LEAF_BITS, memory_block);
      } else {
...
template <typename DataT>
inline SimpleBlockAllocator<DataT>::SimpleBlockAllocator(size_t block_bytes, int blocks_per_chunk)
...
inline DataT* SimpleBlockAllocator<DataT>::allocateBlock() {
  ...
  size_++;
  return reinterpret_cast<DataT*>(&(chunks_.at(chunck_index)->at(block_bytes_ * item_index)));
}
...
inline size_t SimpleBlockAllocator<DataT>::memUsage() const {
  return chunks_.size() * (sizeof(std::vector<char>) + blocks_per_chunk_ * block_bytes_);
}

template <typename DataT>
inline void SimpleBlockAllocator<DataT>::addNewChunk() {
  chunks_.push_back(std::make_shared<std::vector<char>>(blocks_per_chunk_ * block_bytes_));
...
struct hash<Bonxai::CoordT> {
  std::size_t operator()(const Bonxai::CoordT& p) const {
    // same as OpenVDB
    return ((1 << 20) - 1) &
           (static_cast<int64_t>(p.x) * 73856093 ^ static_cast<int64_t>(p.y) * 19349663 ^
