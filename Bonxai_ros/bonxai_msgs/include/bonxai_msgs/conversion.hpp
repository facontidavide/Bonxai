...
void toRosMsg(const Bonxai::VoxelGrid<CellT>& grid, bonxai_msgs::msg::Bonxai& msg)
{
  ...
  msg.raw_data.insert(
      msg.raw_data.end(), data_as_string.begin(), data_as_string.end());
 ...
 */
template <class CellT>
Bonxai::VoxelGrid<CellT> fromRosMsg(const bonxai_msgs::msg::Bonxai& msg,
                                    std::string& type_name = {})
{
  std::istringstream ifile(
      std::string(reinterpret_cast<const char*>(msg.raw_data.data()),
                  msg.raw_data.size()),
      std::ios::binary);
  char header[256];
