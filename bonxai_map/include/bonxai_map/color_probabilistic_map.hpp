...
/// Simple RGB color stored per voxel.
struct Color {
  uint8_t r;
  uint8_t g;
  uint8_t b;
...
/// Extract the RGB color out of any PCL point that exposes r, g, b members
/// (e.g. pcl::PointXYZRGB).
template <typename PointT>
inline Color ExtractColor(const PointT& p) {
  return {p.r, p.g, p.b};
}
...
/**
 * @brief The ColorProbabilisticMap class is meant to behave as much as possible as
 * octomap::Octree but with color, given the same voxel size.
 *
 * Insert a point cloud to update the current probability
 */
class ColorProbabilisticMap {
 public:
  ...
  struct CellT {
    ...
    // running-averaged color of the voxel
    Color color;
    // number of observations averaged into color
  ...
  ColorProbabilisticMap(double resolution);
  ...
  // Accumulate an observed color into the voxel containing point,
  // using a running per-channel average. Called by insertPointCloud
  // for hit points only.
  void updateColor(const Vector3D& point, const Color& color);
  ...
  // Like getOccupiedVoxels(coords), but also returns the averaged color
  // of each occupied voxel (colors[i] corresponds to coords[i]).
  void getOccupiedVoxels(std::vector<Bonxai::CoordT>& coords, std::vector<Color>& colors);
...
inline void ColorProbabilisticMap::insertPointCloud(
  ...
  for (const auto& point : points) {
    ...
    const Color color = ExtractColor(point);
