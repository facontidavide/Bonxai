#include "color_bonxai_server.hpp"
...
ColorBonxaiServer::ColorBonxaiServer(const rclcpp::NodeOptions& node_options)
    : Node("color_bonxai_server_node", node_options) {
  ...
  point_cloud_pub_ = create_publisher<PointCloud2>("color_bonxai_point_cloud_centers", qos);
  ...
  tf_point_cloud_sub_->registerCallback(&ColorBonxaiServer::insertCloudCallback, this);

  reset_srv_ =
      create_service<ResetSrv>("~/reset", std::bind(&ColorBonxaiServer::resetSrv, this, _1, _2));

  // set parameter callback
  set_param_res_ =
      this->add_on_set_parameters_callback(std::bind(&ColorBonxaiServer::onParameter, this, _1));
}

void ColorBonxaiServer::insertCloudCallback(const PointCloud2::ConstSharedPtr cloud) {
  ...
  // Transforming Points to Global Reference Frame (keeps RGB)
  pcl::transformPointCloud(pc, pc, sensor_to_world);

  // Getting the Translation from the sensor to the Global Reference Frame.
  // The origin must be the same point type as the cloud, since insertPointCloud
  // is templated on a single point type; color is irrelevant for the origin.
  const auto& t = sensor_to_world_transform_stamped.transform.translation;

  PCLPoint sensor_to_world_vec3;
  sensor_to_world_vec3.x = t.x;
  sensor_to_world_vec3.y = t.y;
  sensor_to_world_vec3.z = t.z;
  if (max_range_ >= 0) {
...
rcl_interfaces::msg::SetParametersResult ColorBonxaiServer::onParameter(
...
void ColorBonxaiServer::publishAll(const rclcpp::Time& rostime) {
  const auto start_time = rclcpp::Clock{}.now();
  thread_local std::vector<Bonxai::CoordT> bonxai_coords;
  thread_local std::vector<Bonxai::Color> bonxai_colors;
  bonxai_coords.clear();
  bonxai_colors.clear();
  bonxai_->getOccupiedVoxels(bonxai_coords, bonxai_colors);

  if (bonxai_coords.size() <= 1) {
  ...
  if (publish_point_cloud) {
    ...
    for (size_t i = 0; i < bonxai_coords.size(); ++i) {
      const auto voxel = bonxai_->grid().coordToPos(bonxai_coords[i]);
      if (voxel.z >= occupancy_min_z_ && voxel.z <= occupancy_max_z_) {
        PCLPoint point;
        point.x = voxel.x;
        point.y = voxel.y;
        point.z = voxel.z;
        point.r = bonxai_colors[i].r;
        point.g = bonxai_colors[i].g;
        point.b = bonxai_colors[i].b;
  ...
  double total_elapsed = (rclcpp::Clock{}.now() - start_time).seconds();
  RCLCPP_DEBUG(get_logger(), "Map publishing in Bonxai took %f sec", total_elapsed);
}

bool ColorBonxaiServer::resetSrv(
...
RCLCPP_COMPONENTS_REGISTER_NODE(bonxai_server::ColorBonxaiServer)
